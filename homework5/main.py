...
def main():
    ...
    for N in range(1, 30, 1):
        train_X, train_Y = read_train_csv('./data/train.csv', N=N)
        model = LinearRegression()
        model.train(train_X, train_Y)
        ...
        test_X, test_Y = read_valid_csv('./data/validation.csv', N=N)
    ...
    print("best N = ", test_set_loss.index(min(test_set_loss)) + 1)
    N = 4
    try:
